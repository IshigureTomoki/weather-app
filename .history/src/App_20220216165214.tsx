...
function App() {
  ...
  return (
    <div className="App">
      <h1>天気予報</h1>
      <div>現在時刻{weatherReport?.time}</div>
      <div>温度{weatherReport?.temp}</div>
      <div>体感温度{weatherReport?.feels_like}</div>
      <div>風向き{weatherReport?.humidity}</div>
      <div>気圧{weatherReport?.pressure}</div>
      <div>湿度{weatherReport?.temp}</div>
    </div>
  );
}

  ...
  const wetherOneCall = async () => {
    await axios
      ...
      .then((res) => {
        ...
        const hourlyreport = res.data.hourly.map=(a:weatherHourlyType)=>{
          const srTime = new Date(a.dt * 1000);
          const day = srTime.toLocaleDateString();
          const time = srTime.toLocaleTimeString();
          return  a.dt=date_time(a.dt)
        }

        // res.data.hourly[0].dt=date_time(res.data.hourly[0].dt)
        

        console.log(report);
        setWeatherReport(report);
        setWeatherDailyReports(res.data.daily);
        setWeatherHourlyReports(hourlyreport);
      });
  };
  return (
    <div className="App">

...
function App() {
  ...
  const wetherOneCall = async () => {
    await axios
      ...
      .then((res) => {
        ...
        const hourlyreports = res.data.hourly.map((hourly:any, index:number) => {
          return {
            time: time(hourly.dt),
            temp: hourly.temp,
  ...
  return (
    <div className="App">
      ...
      <h3>1時間毎の天気予報</h3>
      <LinePlot weatherHourlyReports={weatherHourlyReports/>
      {}
      {weatherHourlyReports.map((report) => (
        <>
          <div>
            {report.time} {report.temp}℃ {report.humidity}%
            {/* <img
              src={`http://openweathermap.org/img/wn/${report.weather[0].icon}@2x.png`}
              alt="icon"
            /> */}

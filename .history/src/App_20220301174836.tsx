...
function App() {
  ...
  const updatePosition=async()=>{
    try {
      ...
      setCurrentPosition({lat:currentPosition.coords.lat,lon:currentPosition.coords.lon})
    } catch (error) {
  ...
  const wetherOneCall = async () => {
      ...
      await axios
      .get(
        `https://api.openweathermap.org/data/2.5/onecall?lat=${currentPosition.lat}&lon=${currentPosition.lon}&lang=ja&units=metric&appid=ff86fbdda3c22e95ce0071f6a7826f6c`
        )
        .then((res) => {
          console.log(res);

  ...
  const weatherOneCall = async (position: { lat: number; lon: number; }) => {
    await axios
      ...
      .then((res) => {
        ...
        const daylyreports = res.data.dayly.map(
          (daily: any, index: number) => {
          return {
            date: date(daily.dt),
            dayOfWeekStr: dayOfWeekStr(daily.dt),
            weatherIcon: daily.weather[0].weatherIcon,
            description: repdailyort.weather[0].description,
        ...
        console.log("daylyreports");
        // console.log(daylyreports);
        setWeatherDailyReports(res.data.dayly);
